...
@SpringBootTest
@ActiveProfiles("test")
@DisplayName("SlidingWindowCounter Rate Limiter Tests")
public class SlidingWindowCounterRateLimiterTest {
    ...
    @BeforeEach
    void setUp() {
        rateLimiter = new SlidingWindowCounterRateLimiter(redisTemplate, luaScriptExecutor, inMemoryFallback);
    ...
    @Test
    @DisplayName("Should handle sliding window with sub-window accuracy")
    void testSlidingWindowSubWindowAccuracy() {
        String ruleId = "test-accuracy";
        String userId = TestFixtures.TEST_USER_ID;
        long limit = 10;
